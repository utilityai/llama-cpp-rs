...
impl MtmdContext {
    ...
    ///
    /// The embeddings are available after calling `encode_chunk()` on an image
    /// or audio chunk. The size of the embeddings depends on the model and chunk.
    ///
    /// # Returns
    ///
    /// Returns `Some(&[f32])` if embeddings are available, `None` otherwise.
