...
impl State {
    ...
    pub(super) fn is_enabled_for_level(&self, level: llama_cpp_sys_2::ggml_log_level) -> bool {
        ...
        let level = if level == llama_cpp_sys_2::GGML_LOG_LEVEL_CONT {
            self.previous_level.load(std::sync::atomic::Ordering::Relaxed) as llama_cpp_sys_2::ggml_log_level
        } else {
            level
        };
...
mod tests {
    ...
    impl std::io::Write for VecWriter {
        fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
            let log_line= String::from_utf8(buf.to_vec()).map_err(|_| std::io::Error::new(std::io::ErrorKind::InvalidData, "Invalid UTF-8"))?;
    ...
    fn cont_disabled_log() {
        ...
        logs_to_trace(llama_cpp_sys_2::GGML_LOG_LEVEL_DEBUG, c"Hello ".as_ptr(), log_ptr);
        logs_to_trace(llama_cpp_sys_2::GGML_LOG_LEVEL_CONT, c"world\n".as_ptr(), log_ptr);

        assert!(logger.logs.lock().unwrap().is_empty());


        logs_to_trace(llama_cpp_sys_2::GGML_LOG_LEVEL_DEBUG, c"Hello ".as_ptr(), log_ptr);
        logs_to_trace(llama_cpp_sys_2::GGML_LOG_LEVEL_CONT, c"world".as_ptr(), log_ptr);
        logs_to_trace(llama_cpp_sys_2::GGML_LOG_LEVEL_CONT, c"\n".as_ptr(), log_ptr);
    }
    ...
    fn cont_enabled_log() {
        ...
        logs_to_trace(llama_cpp_sys_2::GGML_LOG_LEVEL_INFO, c"Hello ".as_ptr(), log_ptr);
        logs_to_trace(llama_cpp_sys_2::GGML_LOG_LEVEL_CONT, c"world\n".as_ptr(), log_ptr);
