...
// Mirrors what Mistral 3 Reasoning's chat template renders when the caller
// asks for `enable_thinking=false`: the user turn is followed by a closed
// empty `[THINK][/THINK]` block, so generation begins in CONTENT.
...
#[test]
fn mistral3_classifier_does_not_emit_reasoning_for_thinking_disabled_prompt() -> Result<()> {
    ...
    assert_eq!(
        ...
        "Mistral 3 thinking-disabled: classifier must not emit any Reasoning token \
    ...
    assert_eq!(
        ...
        "Mistral 3 thinking-disabled: prompt-token replay must move section to Content \
    ...
    assert_eq!(
        ...
        "Mistral 3 thinking-disabled: usage.reasoning_tokens must be zero; usage={usage:?}"
    );
    assert_eq!(
        usage.undeterminable_tokens, 0,
        "Mistral 3 thinking-disabled: usage.undeterminable_tokens must be zero; usage={usage:?}"
    );
    assert!(
        outcome.observed_content > 0,
        "Mistral 3 thinking-disabled: classifier must emit at least one Content token"
    );
    assert_eq!(
        usage.completion_tokens(),
        outcome.observed_content,
        "Mistral 3 thinking-disabled: completion tokens must equal observed Content tokens"
    );

    for forbidden in FORBIDDEN_MARKERS {
        assert!(
            !outcome.content_stream.contains(forbidden),
            "Mistral 3 thinking-disabled: content_stream leaked marker {forbidden:?}; \
             content_stream={:?}",
