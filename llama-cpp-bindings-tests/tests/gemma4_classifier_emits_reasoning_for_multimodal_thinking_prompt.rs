...
const GEMMA4_MMPROJ_FILE: &str = "mmproj-F16.gguf";

const MAX_GENERATED_TOKENS: i32 = 200;

#[test]
fn gemma4_classifier_emits_reasoning_for_multimodal_thinking_prompt() -> Result<()> {
    let backend = LlamaBackend::init()?;
    require_compiled_backends_present()?;

    let model_path = download_file_from(GEMMA4_REPO, GEMMA4_FILE)?;
    let mmproj_path = download_file_from(GEMMA4_REPO, GEMMA4_MMPROJ_FILE)?;
