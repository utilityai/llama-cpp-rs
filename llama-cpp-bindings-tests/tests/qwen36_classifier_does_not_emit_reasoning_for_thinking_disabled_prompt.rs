...
// Mirrors what Qwen3.6's chat template renders when `enable_thinking=false`:
...
#[test]
fn qwen36_classifier_does_not_emit_reasoning_for_thinking_disabled_prompt() -> Result<()> {
    ...
    assert_eq!(
        ...
        "Qwen3.6 thinking-disabled: classifier must not emit any Reasoning token \
    ...
    assert_eq!(
        ...
        "Qwen3.6 thinking-disabled: usage.reasoning_tokens must be zero; usage={usage:?}"
    );
    assert_eq!(
        usage.undeterminable_tokens, 0,
        "Qwen3.6 thinking-disabled: usage.undeterminable_tokens must be zero; usage={usage:?}"
    );
    assert!(
        outcome.observed_content > 0,
        "Qwen3.6 thinking-disabled: classifier must emit at least one Content token"
    );
    assert_eq!(
        usage.completion_tokens(),
        outcome.observed_content,
        "Qwen3.6 thinking-disabled: completion tokens must equal observed Content tokens"
    );

    for forbidden in FORBIDDEN_MARKERS {
        assert!(
            !outcome.content_stream.contains(forbidden),
            "Qwen3.6 thinking-disabled: content_stream leaked marker {forbidden:?}; \
             content_stream={:?}",
