...
fn main() -> Result<()> {
    ...
    for tokens in &tokens_lines_list {
        ...
        if (batch.n_tokens() as usize + tokens.len()) > 2048 {
            ...
            embeddings_stored += if pooling == "none" { batch.n_tokens() } else { max_seq_id_batch };
            max_seq_id_batch = 0;
            batch.clear();
        }
...
fn batch_decode(
    ...
    // println!("ctx.embeddings_ith(0): {:?}", ctx.embeddings_ith(0));
     ...
     match pooling.as_str() {
        ...
        {  
           for i in 0..batch.n_tokens() {
             if !batch.initialized_logits.contains(&i) {
                // println!("not initialized: {}", i);
                continue;
             } else {
                // println!("initialized: {}", i);
             }
            
          let embeddings =  ctx.embeddings_ith(i).with_context(|| "Failed to get token embeddings")?;
        ...
        _ => {
            for i in 0..s_batch {
                let embeddings = ctx.embeddings_seq_ith(i).with_context(|| "Failed to get sequence embeddings")?;
                let normalized = if normalise {
                    if pooling == "rank" {
